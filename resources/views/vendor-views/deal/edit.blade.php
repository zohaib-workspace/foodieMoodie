                            ...
                            @foreach(json_decode($language) as $lang)
                                <div class="{{$lang != $default_lang ? 'd-none':''}} lang_form" id="{{$lang}}-form">
                                    <div class="form-group mb-0">
                                        ...
                                        <input type="text" {{$lang == $default_lang? 'required':''}} name="name[]" id="{{$lang}}_name" class="form-control h--45px" placeholder="{{translate('Ex : Update Food')}}" value="{{$deal['title']}}" oninvalid="document.getElementById('en-link').click()">
                                    </div>

                                   <div class="form-group mb-0 ">
                                    <label class="form-label" for="exampleFormControlInput1">{{translate('messages.price')}}</label>
                                    <input type="number" min="0" max="100000" step="0.01" value="{{$deal['price']}}" name="price" class="form-control h--45px"
                                            placeholder="{{ translate('Ex : 100') }}" value="{{$deal['price']}}" required>
                                </div>
                                    ...
                                    <div class="form-group pt-4 mb-0">
                                        ...
                                        <textarea type="text" name="description[]" class="form-control ckeditor min-height-154px" placeholder="{{ translate('Ex : Description') }}">{{$deal['description']}}</textarea>
                                    </div>
                                </div>
                            @endforeach
                        @else
                            <div id="{{$default_lang}}-form">
                                <div class="form-group mb-0">
                                    ...
                                    <input type="text" name="name[]" class="form-control h--45px" placeholder="{{translate('Ex : Sepcial Deal')}}" value="{{$deal['title']}}" required>
                                </div>
                                <div class="form-group mb-0 ">
                                    <label class="form-label" for="exampleFormControlInput1">{{translate('messages.price')}}</label>
                                    <input type="number" min="0" max="100000" step="0.01" value="{{$deal['price']}}" name="price" class="form-control h--45px"
                                ...
                                <div class="form-group pt-4 mb-0">
                                    ...
                                    <textarea type="text" name="description[]" class="form-contro ckeditor min-height-154px" placeholder="{{ translate('Ex : Description') }}" value="{{$deal['description']}}"></textarea>
                                </div>
                            </div>
                        @endif
                    </div>
                ...
                <div class="added_products" id="added_products">
                    ...
                    <div class="row pt-4">
                        <div class="col-md-3 pt-3">
                            <input type="text" class="form-control h--45px" name="items[{{$item['food_id']}}][name][]" value="{{$item['name']}}" placeholder="{{translate('messages.choice_title')}}" readonly>
                            <input type="hidden" name="items[{{$item['food_id']}}][prods_ids][]" value="{{$item['id']}}">
                        </div>
                        <?php
                        $vars = [];
                        $opts = \App\Models\DealOptions::where('deal_id', $deal['id'])->where('food_id', $item['food_id'])->select('value')->get();
                        if($opts) {
                            foreach($opts as $t) {
                                $vars[] = $t['value'];
                            }
                        ...
                        <div class="col-md-6 pt-3">
                            <input type="text" class="form-control h--45px" value="{{ implode(',', $vars)}}" name="items[{{$item['food_id']}}][options][][]" placeholder="{{translate('Add Choices')}}" data-role="tagsinput">
                        </div>
                        <div class="col-md-3 pt-3">
...
@push('script_2')
    ...
    <script>
        $('#food_form').on('submit', function () {
            ...
            $.post({
                url: '{{route('vendor.deal.update',[$deal['id']])}}',
                data: $('#food_form').serialize(),
                data: formData,
                cache: false,
                contentType: false,
