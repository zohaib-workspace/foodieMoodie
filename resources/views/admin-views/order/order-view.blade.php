...
@section('content')
                    ...
                    <div class="card-header border-0 align-items-start flex-wrap">
                        <div class="order-invoice-left">
                            ...
                            <h6 class="mt-2 pt-1 mb-2">
                                <i class="tio-star"></i>
                                {{ translate('messages.Rating') }} :
                                @if ($order->order_status == 'delivered')


                                    <label>
                                        <!-- Rating -->
                                        @if (\App\Models\OrderReview::where('order_id', $order['id'])->exists())
                                            ...
                                            <span class="rating">
                                                ...
                                                <i class="tio-star" style="opacity: 0.5"></i>
                                                <?php } ?>
 ...
 @if($order['order_type'] != 'take_away')
                                                    ...
                                                    <a class="dropdown-item {{ $order['order_status'] == 'picked_up' ? 'active' : '' }}"
                                                        ...
                                                        <a class="dropdown-item {{ $order['order_status'] == 'rider_accepted' ? 'active' : '' }}"
                                                        onclick="route_alert('{{ route('admin.order.status', ['id' => $order['id'], 'order_status' => 'rider_accepted']) }}','{{ translate('Change status to rider_accepted ?') }}')"
                                                        href="javascript:">{{ translate('messages.rider_accepted') }}</a>
                                                        <a class="dropdown-item {{ $order['order_status'] == 'arrived_at_vendor' ? 'active' : '' }}"
                                                        onclick="route_alert('{{ route('admin.order.status', ['id' => $order['id'], 'order_status' => 'arrived_at_vendor']) }}','{{ translate('Change status to arrived_at_vendor ?') }}')"
                                                        href="javascript:">{{ translate('messages.arrived_at_vendor') }}</a>
                                                        <a class="dropdown-item {{ $order['order_status'] == 'arrived_at_customer' ? 'active' : '' }}"
                                                        onclick="route_alert('{{ route('admin.order.status', ['id' => $order['id'], 'order_status' => 'arrived_at_customer']) }}','{{ translate('Change status to arrived_at_customer ?') }}')"
                                                        href="javascript:">{{ translate('messages.arrived_at_customer') }}</a>
@endif        
                            ...
                            <ul class="list-group overflow-auto max-height-400">
                                ...
                                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                    <button class="nav-link active" id="nav-home-tab" data-bs-toggle="tab"
                                        ...
                                        aria-selected="true">{{ translate('messages.Absher_Rider') }}</button>
                                    <button class="nav-link" id="nav-profile-tab" data-bs-toggle="tab"
                                        data-bs-target="#nav-profile" type="button" role="tab"
                                        aria-controls="nav-profile"
                                        aria-selected="false">{{ translate('messages.Freelancer') }}</button>
                                </div>
                            </nav>
                            <div class="tab-content" id="nav-tabContent">
                                    ...
                                    <ul class="list-group overflow-auto max-height-400">
                                        ...
                                        @foreach ($deliveryMen as $dm)
                                            @if ($dm['dm_type'] == 0)
                                                ...
                                                <?php $deliveryManFound = true ?>
                                            @endif
                                        @endforeach
                                        @if (!$deliveryManFound)
                                            <li class="list-group-item">{{ translate('messages.No_Absher_Rider_Found') }}
                                            </li>
                                        @endif
                                    </ul>
                                </div>
                                <div class="tab-pane fade" id="nav-profile" role="tabpanel"
                                    ...
                                    <ul class="list-group overflow-auto max-height-400">
                                        ...
                                        @endforeach
                                        @if (!$deliveryManFound)
                                            <li class="list-group-item">{{ translate('messages.No_Freelancer_Found') }}
                                            </li>
                                        @endif
                                    </ul>
                                </div>
