...
@section('title', translate('Add Privacy Policy'))
            ...
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    <h1 class="page-header-title"><i class="tio-add-circle-outlined"></i> {{ translate('messages.add') }}
                        {{ translate('messages.new') }} {{ translate('messages.privacy_policy') }}</h1>
                </div>
            </div>
        </div>
        <!-- End Page Header -->
                                        ...
                                        <div id="{{ $default_lang }}-form">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="title" class="form-control"
                                                    placeholder="{{ translate('messages.privacy_policy') }}" required>
                                            </div>
                            ...
                            <div class="card h-10">
                                <div class="card-header">
                                    <h5 class="card-title">
                                        ...
                                        <span>{{ translate('privacy_policy Image') }} <small class="text-danger">({{ translate('messages.Ratio 200x200') }})</small></span>
                                    </h5>
                                </div>
                                <div class="card-body">
...
@push('script_2')
    ...
    <script>
        $('#food_form').on('submit', function(e) {
            ...
            $.post({
                url: '{{ route('admin.privacy_policy.store') }}',
                ...
                success: function(data) {
                    ...
                    if (data.errors) {
                        ...
                        toastr.success('{{ translate('messages.privacy_policy_added_successfully') }}', {
                            CloseButton: true,
                            ProgressBar: true
                        });
                        setTimeout(function() {
                            location.href =
                                '{{ \Request::server('HTTP_REFERER') ?? route('admin.privacy_policy.list') }}';
                        }, 2000);
                    }
                }
            });
