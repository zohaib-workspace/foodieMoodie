...
@section('title', translate('Express'))
            ...
            <div class="col-sm-12 col-lg-12 mb-3 mb-lg-2">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">{{ translate('messages.Absher_Express') }} {{ translate('messages.list') }}<span
                                class="badge badge-soft-dark ml-2" id="itemCount">{{ $pickups->count() }}</span></h5>
                        <form id="search-form">
                            @csrf
                            <!-- Search -->
                        ...
                        <table id="columnSearchDatatable"
                            ...
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th>{{ translate('messages.Pickup_Name') }}</th>
                                    <th>{{ translate('messages.DropOff_Name') }}</th>
                            ...
                            <tbody id="set-rows">
                                @foreach ($pickups as $key => $pickup)
                                    <tr>
                                        <td>{{ $key + $pickups->firstItem() }}</td>
                                        <td>
                                            {{ translate('messages.' . $pickup['pickup_name']) }}

                                        </td>
                                        <td>{{ translate('messages.' . $pickup['dropoff_name']) }}</td>
                                        <td>
                                            {{ $pickup['price'] }}
                                        </td>
                                        <td> </td>
                                        <td>
                                            ...
                                            @elseif($pickup['status'] == 'confirmed')
                                                <span class="badge badge-soft-info mb-1">
                                                    {{ translate('messages.confirmed') }}
                                                </span>
                                            @elseif($pickup['status'] == 'processing')
                                            ...
                                            @else
                                                <span class="badge badge-soft-danger mb-1">
                                                    {{ translate(str_replace('_', ' ', $pickup['status'])) }}
                                                </span>
                                            @endif
                                        </td>
                                        <td>
                                            ...
                                            onclick="openChat(this.id)"><i class="tio-chat"></i></button> --}}
                                                <button type="button" class="btn action-btn btn--warning btn-outline-warning" ><i
                                                        class="tio-visible-outlined" data-toggle="modal" data-target="#exampleModal{{$pickup['id']}}"></i></button>
                                                <a class="btn action-btn btn--primary btn-outline-primary" 
                                                    href="{{ route('admin.business-settings.express-update', [$pickup['id']]) }}"><i class="tio-edit"></i></a>
                                            </div>
                                            <!-- Modal -->
                                            <div class="modal fade" id="exampleModal{{$pickup['id']}}" tabindex="-1" role="dialog"
                                                aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title" id="exampleModalLabel{{$pickup['id']}}">{{$pickup['pickup_name']}}</h5>
                                                        ...
                                                        <div class="modal-body">
                                                            <div class="container">
                                                                <div class="row">
                                                                    <div class="col h4">{{ translate('Pickup_Phone_No.') }}</div>
                                                                    <div class="col h4">{{$pickup['pickup_phone']}}</div>
                                                                </div>
                                                                <div class="row">
                                                                    <div class="col-6 h4">{{ translate('Pickup_Address') }}</div>
                                                                    <div class="col-6 overflow-auto ">{{$pickup['pickup_address']}}</div>
                                                                </div>
                                                                <div class="row">
                                                                    <div class="col h4">{{ translate('Drop_Off_Name') }}</div>
                                                                    <div class="col h4">{{$pickup['dropoff_name']}}</div>
                                                                </div>
                                                                <div class="row">
                                                                    <div class="col h4">{{ translate('Drop_Off_Phone_No.') }}</div>
                                                                    <div class="col h4">{{$pickup['dropoff_phone']}}</div>
                                                                </div>
                                                                <div class="row">
                                                                    <div class="col-6 h4">{{ translate('Drop_Off_Address') }}</div>
                                                                    <div class="col-6 overflow-auto">{{$pickup['dropoff_address']}}</div>
                                                                </div>
                                                                <div class="row">
                                                                    <div class="col h4">{{ translate('Price') }}</div>
                                                                    <div class="col h4">{{$pickup['price']}}</div>
                                                                </div>
                                                                <div class="row">
                                                                    <div class="col h4">{{ translate('Category') }}</div>
                                                                    <div class="col h4">{{$pickup->category->name}}</div>
                                                                </div>
                                                                <div class="row">
                                                                    <div class="col h4">{{ translate('Status') }}</div>
                                                                    <div class="col h4">
                                                                        <p
                                                                            class="h4 {{ $pickup->status == 'pending' ? 'badge badge-info' : '' }} {{ $pickup->status == 'Active' ? 'badge badge-primary' : '' }} {{ $pickup->status == 'Ended' ? 'badge badge-success' : '' }}">
                                                                            {{ $pickup['status'] }}
                                                                        </p>
                                                                    </div>
                                                                </div>
