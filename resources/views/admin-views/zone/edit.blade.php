                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 col-xl-7 zone-setup">
                         ...
                         <div class="col-md-6 col-lg-4 col-12">
                            <div class="form-group">
                                    <label class="input-label" for="choice_types">{{ translate('messages.currency ') }}<span
                                                class="input-label-secondary"
                                                title="{{ translate('messages.select_currency_types') }}"><img
                                                    src="{{ asset('/public/assets/admin/img/info-circle.svg') }}"
                                                    alt="{{ translate('messages.select_currency_type') }}"></span></label>
                                        <select name="currency_id" id="choice_types" 
                                            data-placeholder="{{ translate('messages.select') }} {{ translate('messages.currency') }}"
                                            class="form-control h--45px js-select2-custom">
                                            @foreach (\App\Models\Currency::all() as $c_id)
                                                    <option value="{{ $c_id->id }}"
                                                        {{ $zone->currency_id == $c_id->id ? 'selected' : '' }}>
                                                        {{ $c_id->country }}<span> </span>{{$c_id->currency_symbol}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <label class="input-label" for="choice_types">{{ translate('messages.Time Zone') }}<span
                                        ...
                                        <select name="timezone_id" id="choice_zone" 
                                            ...
                                            @foreach (\App\Models\Timezone::all() as $t_z)
                                                    <option value="{{ $t_z->id }}"
                                                        {{ $zone->timezone_id == $t_z->id ? 'selected' : '' }}>
                                                        {{ $t_z->timezone }}</option>
                                            @endforeach
                                        </select>
                                    </div>
