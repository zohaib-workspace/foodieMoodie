                ...
                <form action="{{route('admin.zone.store')}}" method="post" class="shadow--card">
                    ...
                    <div class="row justify-content-between">
                        ...
                        <div class="col-md-6 col-xl-7 zone-setup">
                            ...
                            <label class="input-label" for="choice_business">{{ translate('messages.Currency') }}
                                        <span data-toggle="tooltip" data-placement="right"
                                        class="input-label-secondary"><img
                                            src="{{ asset('/public/assets/admin/img/info-circle.svg') }}"
                                            alt="{{ translate('messages.currency_lat_lng_warning') }}"></span>
                                        </label>                               
                                        <select name="currency_id" id="choice_currency" required class="form-control h--45px js-select2-custom"
                                    data-placeholder="{{ translate('messages.select') }} {{ translate('messages.Currency') }}" required> 
                                    <option value="" selected disabled>{{ translate('messages.select') }}
                                        {{ translate('messages.Currency') }}</option>                               
                                    @foreach (\App\Models\Currency::all() as $c_id)                                     
                                            <option value="{{ $c_id->id }}">{{ $c_id->country }}<span> </span>{{$c_id->currency_symbol}}</option>                               
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <table id="columnSearchDatatable"
                            ...
                            <tr>
                                ...
                                <th class="pl-5">{{translate('messages.name')}}</th>
                                 <th class="pl-5">{{translate('messages.timezone')}}</th>
                                  <th class="pl-5">{{translate('messages.currency')}}</th>
                                <th class="pl-5">{{translate('messages.currency_symbol')}}</th>
                            ...
                            @foreach($zones as $key=>$zone)
                                <tr>
                                    ...
                                    <td class="pl-5">
                                        <span class="d-block font-size-sm text-body">
                                            {{$zone['currenci']->country}}
                                        </span>
                                    </td>
                                    <td class="pl-5">
                                        <span class="d-block font-size-sm text-body">
                                            {{$zone['currenci']->currency_symbol}}
                                        </span>
                                    </td>
                                    <td class="text-center">
                                        <span class="move-left">
