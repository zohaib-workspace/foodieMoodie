...
class CategoryLogic
{
    ...
    public static function all_products($id)
    {
     ...
     public static function categories_products($ids,$restaurant_id)
    {
        $cate_ids=$ids;
        return Category::with(['category_products'=>function($q) use ($restaurant_id) {
            $q->where('restaurant_id',$restaurant_id);
        }])->whereIn('id', $cate_ids)->get();
    }
    public static function categories_products2($id,$restaurant_id)
    {
        // $cate_ids=$ids;
        return Category::with(['category_products'=>function($q) use ($restaurant_id) {
            $q->where('restaurant_id',$restaurant_id);
