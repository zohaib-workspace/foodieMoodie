...
class ProductLogic
{
    ...
    {
        // var_dump(Food::active()->where('id', $id)->with(['favorites' => function ($query) use($user_id) {
        //     $query->where('user_id', 2);
        //     }])->select(['food.*', \DB::raw('IF(id IS NULL, 0, 1) as is_favorite')])->first());
        return Food::active()->where('id', $id)
            // ->where('id', $id)->with(['favorites' => function ($query) {
            // $query->where('user_id', $user_id);
            // }])->select(['food.*', \DB::raw('IF(id IS NULL, 0, 1) as is_favorite')])
            ->first();
    }
    public static function get_products($ids)
    {
        return Food::active()->whereIn('id', $ids)->get();
    }
    ...
    public static function popular_products($zone_id, $limit = null, $offset = null, $type = 'all', $restaurantId = '')
    {
        if ($limit != null && $offset != null) {
            if ($restaurantId == 0) {
                ...
                $paginator = Food::whereHas('restaurant', function ($q) use ($zone_id, $restaurantId) {
                    $q->whereIn('zone_id', $zone_id)->where('restaurant_id', $restaurantId)->Weekday();
                })->active()->type($type)->popular()->paginate($limit, ['*'], 'page', $offset);
            }
