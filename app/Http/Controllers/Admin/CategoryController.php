...
class CategoryController extends Controller
{
    function index()
    {
        $categories=Category::where(['position'=>0])->latest()->paginate(config('default_pagination'));
         $params = [
            'business_type' => $request['business_type'] ?? 'all',
        ];
        
        session()->put('cate_params', $params);
        return view('admin-views.category.index',compact('categories','params'));
    }
    ...
    public function search(Request $request){
        ...
        when($request->sub_category, function($query){
         ...
         ->when(is_numeric($business_type), function($query)use($business_type){
