    ...
    public function details(Request $request, $id)
    {
        ...
        if (isset($order)) {
            if (isset($order->restaurant) && $order->restaurant->self_delivery_system) {
                $deliveryMen = DeliveryMan::where('earning',0)->available()->active()->get();
            } else {
                if($order->restaurant !== null){
                    // $deliveryMen = DeliveryMan::where('zone_id', $order->restaurant->zone_id)->where('earning',0)->available()->active()->get();
                     $deliveryMen = DeliveryMan::where('zone_id', $order->restaurant->zone_id)->absherrider()->available()->active()->get();
