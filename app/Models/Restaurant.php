...
class Restaurant extends Model
{
    ...
    {
        $query->select('*',\DB::raw('IF(((select count(*) from `restaurant_schedule` where `restaurants`.`id` = `restaurant_schedule`.`restaurant_id` and `restaurant_schedule`.`day` = '.now()->dayOfWeek.' and `restaurant_schedule`.`opening_time` < "'.now()->format('H:i:s').'" and `restaurant_schedule`.`closing_time` >"'.now()->format('H:i:s').'") > 0), true, false) as open'));
    ...
    public function scopeOnlyOpen($query){
        $query->whereRaw("(select count(*) from restaurant_schedule where restaurants.id = restaurant_schedule.restaurant_id and restaurant_schedule.day = '".now()->dayOfWeek."' and restaurant_schedule.opening_time < '".now()->format('H:i:s')."' and restaurant_schedule.closing_time > '".now()->format('H:i:s')."') > 0");
    }
    ...
    public function scopeBusinessType($query, $businessType){
